...
public class NfvDeployer {
	...
	public synchronized boolean allocateNode(String nodeName, String preferenceHost)
		...
		if (preferenceHost != null) {
			...
			if (requiredMemory < availableMemory && requiredStorage < availableStorage && deployedVnfs < maxVnfs) {
				// allocate node
				// hostMap.get(hostIds.get(preferenceHost)).setAllocatedMemory(BigInteger.valueOf(allocatedMemory
				// + requiredMemory));
				// hostMap.get(hostIds.get(preferenceHost)).setAllocatedStorage(BigInteger.valueOf(allocatedStorage
				// + requiredStorage));
				hostMap.get(hostIds.get(preferenceHost)).getDeployedNodes().getNode()
						.add(createNamedRef(nodeName, n.getHref()));
				...
				this.dataApi.nodeNodeidRelationships(this.nodeIds.get(nodeName)).postXml(allocatedOn,
						it.polito.dp2.NFV.sol3.service.data.Relationship.class);
				return true;
			}
		}
		...
		for (Host h : hostMap.values()) {
			...
			if (requiredMemory < availableMemory && requiredStorage < availableStorage && deployedVnfs < maxVnfs) {
				// allocate node
				// hostMap.get(hostIds.get(h.getName())).setAllocatedMemory(BigInteger.valueOf(availableMemory
				// - requiredMemory));
				// hostMap.get(hostIds.get(h.getName())).setAllocatedStorage(BigInteger.valueOf(availableStorage
				// - requiredStorage));
				hostMap.get(hostIds.get(h.getName())).getDeployedNodes().getNode()
						.add(createNamedRef(nodeName, n.getHref()));
	...
	public Connection createConnection(String source, String destination, int latency, float throughput)
		...
		if (throughput < 0)
			throw new InvalidEntityException("Throughput cannot be negative");
	...
	public synchronized String createHost(String hostName, Integer availableMemory, Integer availableStorage,
	...
	private boolean isDuplicateLink(String graph, String srcNode, String dstNode) {
		...
		if (!nffgMap.containsKey(graph))
			return false;
		if (!isExistingNode(srcNode) || !isExistingNode(dstNode))
			return false;
		if (!linkNames.containsKey(graph))
			return false;

		for (Link l : nffgLinks.get(graph).getLink()) {
			if (l.getSrc().equals(srcNode) && l.getDst().equals(dstNode))
				return true;
		}
	...
	public synchronized Node createNode(String nffg, String nodeName, String type, boolean replace)
		...
		if (nffg == null)
			throw new InvalidEntityException("NF-FG name not provided");
		if (nodeName == null)
			throw new InvalidEntityException("Node name not provided");
		if (type == null)
	...
	private synchronized boolean deleteAllocation(String nodeName) {
		if (!isExistingNode(nodeName))
		...
		if (n.getHost() != null) { // node has been allocated on host
			...
			if (isExistingHost(n.getHost().getName())) {
				 ...
				 * Vnf v = vnfs.get(n.getVnf().getName());
				 * 
				 * int requiredMemory = (v.getRequiredMemory() != null) ?
				 * v.getRequiredMemory().intValue() : 0; int requiredStorage =
				 * (v.getRequiredStorage() != null) ? v.getRequiredStorage().intValue() : 0; int
				 * availableMemory = (h.getAvailableMemory() != null) ?
				 * h.getAvailableMemory().intValue() : 0; int availableStorage =
				 * (h.getAvailableStorage() != null) ? h.getAvailableStorage().intValue() : 0;
				 * 
				 * hostMap.get(id).setAvailableMemory(BigInteger.valueOf(availableMemory -
				 * requiredMemory));
				 * hostMap.get(id).setAvailableStorage(BigInteger.valueOf(availableStorage -
				 * requiredStorage));
				 */
		...
		for (Relationship r : rels.getRelationship()) {
			if (r.getType().equals("AllocatedOn")) {
				try {
					...
					// ignore service exceptions, they won't cause trouble
					e.printStackTrace();
				}
			}
		}
	...
	private synchronized boolean deleteLinks(String graph, String node) {
	...
	public synchronized boolean deleteNffg(String name) throws UnknownEntityException {
	...
	public synchronized boolean deleteNode(String graph, String n) throws UnknownEntityException {
	...
	public Hosts getReachableHosts(String graph, String nodeName) throws UnknownEntityException, ServiceException {
		...
		it.polito.dp2.NFV.sol3.service.data.Nodes reachableHosts = dataApi
				.nodeNodeidReachableNodes(nodeIds.get(nodeName)).getAsNodes(null, "Host");
	...
	public boolean isExistingNode(String name) {
		...
		if (!nodeIds.containsKey(name))
			return false;
		if (!nodeMap.containsKey(nodeIds.get(name)))
			return false;
	...
	public boolean isExistingHost(String name) {
		...
		if (!hostIds.containsKey(name))
			return false;
		if (!hostMap.containsKey(hostIds.get(name)))
			return false;
	...
	public boolean isExistingLink(String graph, String name) {
		...
		if (!linkNames.containsKey(graph))
			return false;
		if (!linkNames.get(graph).containsKey(name))
			return false;
		return true;
	}
